...
/* Stream operators */
ostream& operator<< (ostream& out, Ipoint& ip)
{
	out << ip.x <<" " << ip.y <<" "<< ip.scale<<" "<< ip.orientation <<" "<< ip.laplacian <<" "<< ip.dx <<" "<< ip.dy <<" "<< ip.clusterIndex << endl;  
...
istream& operator>> (istream& in, Ipoint& ip)
{
	in >> ip.x >> ip.y >> ip.scale >> ip.orientation >> ip.laplacian >> ip.dx >> ip.dy >> ip.clusterIndex;  
	for (unsigned int i=0; i<64; ++i)
		in >> ip.descriptor[i];
...
ostream& operator<< (ostream& out, IpVec& ipv)
{
	out << ipv.size() << endl;
...
istream& operator>> (istream& in, IpVec& ipv)
{
	int size;
	in >> size;
	ipv.reserve(size);
...
void saveIpVec(char* file, IpVec& ipv)
{
	ofstream ofs(file);
	ofs << ipv;
	ofs.close();	
